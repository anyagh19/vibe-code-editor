...
const PlaygroundPage = () => {
  ...
  const handleSave = useCallback(async (fileId: string) => {
    ...
    try {
      ...
      if (!filePath) {
        toast.error(`Could not find oath for File: ${fileToSave.filename} . ${fileToSave.fileExtension}`)
        return;
      }
      ...
      const updatedFileContent = (items: any[]) => {
        items.map((item) => {
          if ("folderName" in item) {
            return { ...item, item: updatedFileContent(item.items) }
          }
          else if (
